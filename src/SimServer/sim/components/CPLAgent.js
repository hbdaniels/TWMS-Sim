...
function buildCPLExitMessage(deposits) {
  const segments = deposits.map((coil, index) => {
    if (!coil) return '               ;00000;0000;0000;00000';
  ...
  return `*${formatDate(new Date())};00901;0000;CPL  ;TWMS Data:${segments.join(';')}#`;
}
...
export class CPLAgent {
  constructor({ rate = 1, campaign = {}, socketServer = null, createInterval = 60 }) {
    this.rate = rate;
    this.counter = 0;
  ...
  async tick(state) {
    ...
    // ✅ Try to unload the transfer car into a valid deposit (5 → 3)
    if (this.transferCar) {
      for (let i = 2; i >= 0; i--) {
    ...
    // ✅ Debounced CPL exit message
    const now = Date.now();
    if (changed && now - this.lastSentAt > 1000) {
